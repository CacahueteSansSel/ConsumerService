...
public class ProtectiveShelfContainer extends Container {

    public static ProtectiveShelfContainer newServer(int windowID, PlayerInventory playerInventory, ItemStackHandler inv) {
        return new ProtectiveShelfContainer(windowID, playerInventory, inv);
    }

    public static ProtectiveShelfContainer newClient(int windowID, PlayerInventory playerInventory, net.minecraft.network.PacketBuffer extraData) {
        ...
        ItemStackHandler handler = new ItemStackHandler(PlayerInventorySlotCount);
        ...
        return new ProtectiveShelfContainer(windowID, playerInventory, handler);
    }
     ...
     */
    private ProtectiveShelfContainer(int windowID, PlayerInventory playerInventory, ItemStackHandler inv) {
        super(ConsumerContainers.ContainerProtectiveShelf, windowID);
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                addSlot(new Slot(wrap, slotIdx, TILE_INVENTORY_XPOS + 1 + 18 * x, TileEntityInventoryY + 1 + 18 * y));
                slotIdx++;
            }
        }
    }
