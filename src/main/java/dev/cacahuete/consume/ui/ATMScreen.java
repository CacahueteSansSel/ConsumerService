...
public class ATMScreen extends Screen {
    ...
    void buttonRelogin(Button btn) {
        minecraft.displayGuiScreen(new BankPortalLoginScreen((scr, packet) -> {
            if (packet.isError) {
                minecraft.displayGuiScreen(new BankPortalResponseScreen(packet));
            } else {
                token = packet.token;
                wallet = packet.wallet;
    ...
    public class TransferMenu extends MenuBase {
        ...
        public void continueButtonClicked() {
            ...
            AccountManager.ATM.send(token, targetWalletIDField.getText(), amount, (packet) -> {
                ...
                minecraft.displayGuiScreen(new BankPortalResponseScreen(packet).withFallbackScreen(screen));
                return true;
            });
        }
    }
