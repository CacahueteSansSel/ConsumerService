...
public class CryptoLoginScreen extends Screen {
    private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation("consume", "textures/gui/crypto_login.png");
    private static final ResourceLocation CARD_BUTTON_TEXTURE = new ResourceLocation("consume", "textures/gui/card_button.png");
    ...
    public CryptoLoginScreen(CryptoLoginResponseListener listener) {
    ...
    void initUI() {
        ...
        buttonCard = new ImageButton(windowX + 142, windowY + 40, 18, 18, 0, 0, 18, CARD_BUTTON_TEXTURE, (btn) -> {
            ClientPlayerEntity player = minecraft.player;
            ItemStack card = null;
            for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
                ItemStack cur = player.inventory.getStackInSlot(i);
    ...
    void renderForeground(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        drawTexture(matrixStack, BankResources.BANK_LOGO_LOCATION, windowX + 10, windowY + 132, 128, 128, 128, 128);
    ...
    public interface CryptoLoginResponseListener {
        void listen(CryptoLoginScreen screen, ServerToClientCryptoLoginResponsePacket packet);
    }
}
